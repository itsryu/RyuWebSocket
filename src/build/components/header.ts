...
export function renderHeader(data: UserProfileResponse, member: MemberPresence): string {
    const username = data.user.global_name ?? data.user.username;
    const avatar = data.user.avatar
        ? `${process.env.DISCORD_CDN}/avatars/${data.user.id}/${data.user.avatar}.png?size=4096`
        : `${process.env.DISCORD_CDN}/embed/avatars/0.png`;

    const avatarDecoration = data.user.avatar_decoration_data?.asset
        ? `${process.env.DISCORD_CDN}/avatar-decoration-presets/${data.user.avatar_decoration_data.asset}.png?size=512`
        : null;
    ...
    const clanBadge = clan
        ? `${process.env.DISCORD_CDN}/clan-badges/${clan.identity_guild_id}/${clan.badge}.png?size=512`
        : null;

    const badges = data.badges.map((badge) => `${process.env.DISCORD_CDN}/badge-icons/${badge.icon}.png`);
        ...
        <div style="height:80px;width:260px;display:flex;flex-direction:column;justify-content:center">
            <div style="display:flex;flex-direction:row;height:25px">
                ...
                <span style="background-color:#111214;border-radius:0.375rem;padding-left:0.5rem;padding-right:0.5rem;margin-left:-6px;margin-right:12px;display:flex;align-items:center;gap:0.25rem;font-size:16px;font-weight:500;font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;height:100%">
                    ${clanBadge ? `<img src="${clanBadge}" alt="Clan Badge" style="width:16px;height:16px;" />` : ''}
                    <p style="margin-bottom:1.1rem;white-space:nowrap">${escapeXml(clan.tag)}</p>
                </span>` : ''}
                ${badges.map(badge => `
                <img src="${badge}" alt="User Badge" style="width:auto;height:20px;position:relative;top:50%;transform:translate(0%, -50%);margin-right:4px" />
                `).join('')}
            </div>
            ${customStatus ? `
